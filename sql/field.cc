...
/*
  Compressing double and longlong is pointless, thus we don't even attempt to
  compress them.

  According to Field_blob::store() code we can store value directly if:
  - data is stored in value and
  - value.length() > 0 and
  - value.length() <= max_data_length() and
  - no character set conversion is needed and
  - table->blob_storage is not set
*/

int Field_varstring_compressed::store(double nr)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE_OR_COMPUTED;
  char *buf= (char*) get_data();
  size_t length;

  buf[0]= 0;
  length= my_gcvt(nr, MY_GCVT_ARG_DOUBLE, field_length - 2, buf + 1, NULL);
...
int Field_varstring_compressed::store(longlong nr, bool unsigned_val)
{
  ...
  *buf= 0;
  length= (field_charset->cset->longlong10_to_str)(field_charset,
                                                   buf + 1, field_length - 1,
                                                   unsigned_val ? 10 : -10, nr);
  store_length(length + 1);
  return 0;
}
                                       ...
                                       const uchar *from, size_t from_length)
{
  ...
  if (!algorithm)
  {
    val_ptr->set((const char*) from, from_length, charset());
    return val_ptr;
  }
...
{
  char buf[FLOATING_POINT_BUFFER + 1];
  uint dummy_errors;
  size_t length;

  DBUG_ASSERT(!table->blob_storage);
  buf[0]= 0;
  length= my_gcvt(nr, MY_GCVT_ARG_DOUBLE, sizeof(buf) - 2, buf + 1, NULL);
  value.copy(buf, length + 1, &my_charset_latin1, field_charset, &dummy_errors);
  set_ptr(value.length(), (uchar*) value.ptr());
  return 0;
}
...
int Field_blob_compressed::store(longlong nr, bool unsigned_val)
{
  ...
  *buf= 0;
  length= (field_charset->cset->longlong10_to_str)(field_charset,
                                                   buf + 1,
                                                   value.alloced_length() - 1,
                                                   unsigned_val ? 10 : -10, nr);
  value.length(length + 1);
  value.set_charset(field_charset);
  set_ptr(value.length(), (uchar*) value.ptr());
  return 0;
}
